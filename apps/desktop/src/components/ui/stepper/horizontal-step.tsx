...
const horizontalStep = forwardRef<HTMLDivElement, StepSharedProps>((props, reference) => {
  const {
    isError,
    isLoading,
    onClickStep,
  ...
  return (
    <div
      ...
      className={cn(
        ...
        "[&:not(:last-child)]:after:content-[''] [&:not(:last-child)]:after:h-[2px] [&:not(:last-child)]:after:bg-stepper-muted",
        ...
        variant === "line" && "flex-col flex-1 border-t-[3px] data-[active=true]:border-stepper",
    ...
    >
      ...
      >
        ...
        <StepLabel description={description} label={label} {...{ isCurrentStep, opacity }} />
      </div>
    </div>
  );
});
